...
This project implements a supervised, transformer-based multimodal Fake News Detection System that classifies news as Fake or Real using a Random Forest classifier trained on semantic embeddings. During training (ST.ipynb), two labeled datasets, Fake.csv (label = 1) and True.csv (label = 0), are combined, and the title and text of each article are merged into a single input. Basic text preprocessing such as special character removal and lowercasing is applied. Each news article is then converted into a 768-dimensional semantic embedding using the pre-trained Sentence Transformer model all-mpnet-base-v2, which captures the contextual meaning of the text. These embeddings are used to train a Random Forest Classifier with 200 decision trees using an 80–20 stratified train-test split. The trained model is evaluated using accuracy, precision, recall, F1-score, and a confusion matrix, and finally saved as fake_news_classifier.pkl for deployment. During real-time prediction (app.py), the system accepts multimodal inputs including text, images, audio, YouTube videos, and live RSS news. Image inputs are converted to text using OCR (pytesseract), audio and YouTube inputs are converted to text using Whisper speech-to-text, and RSS news is fetched using feedparser only for live testing. All extracted text is passed through the same preprocessing and embedding pipeline before being classified by the trained Random Forest model. The system does not use cosine similarity, text-to-text matching, or live source comparison; instead, it performs direct supervised classification based on patterns learned from the fixed labeled dataset. The final output is a binary decision indicating whether the given news is Fake or Real.
